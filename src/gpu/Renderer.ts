import { BRUSH_WGSL, SHAPE_WGSL, BLIT_WGSL, OVERLAY_WGSL } from "./shaders";
...
export class Renderer {
  ...
  clearFrame(frame: FrameTexture, color: RGB): void {
    const encoder = this.device.createCommandEncoder();
    const pass = encoder.beginRenderPass({
      colorAttachments: [
        {
          view: frame.view,
          clearValue: { r: color.r, g: color.g, b: color.b, a: 1 },
          loadOp: "clear",
          storeOp: "store",
        },
      ],
  ...
  blitFrameFill(target: HTMLCanvasElement, frame: FrameTexture): void {
